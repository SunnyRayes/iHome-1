...
var house_data_querying = false;   // 是否正在向后台获取数据
...
function updateHouseData(action) {
    ...
    console.log(params);
    // TODO: 获取房屋列表信息
    $.get('/api/1.0/houses', params, function (response) {
            if (response.errno === '0') {
                if (action === 'renew') {
                    $('.house-list').html(template('house-list-tmpl', {houses: response.data.houses}));
                }
                else {
                    $('.house-list').append(template('house-list-tmpl', {houses: response.data.houses}));
                }
